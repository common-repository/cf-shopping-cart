...
function cfshoppingcart_cart($args = '') {
    ...
    foreach ($commodities as $postid => $commodity) {
        ...
        // if (function_exists('cf_qf_get_thumb_one')) {
        if (function_exists('the_qf_get_thumb_one') && $model->getShowThumbnailImageInCartScreen()) {
            //$img = '<td rowspan="' . (count($custom_fields)+2) . '">' . cf_qf_get_commodity_img($postid, $qfgetthumb_option_1, $qfgetthumb_default_image) . '</td>';
            $img = cf_qf_get_commodity_img($postid, $qfgetthumb_option_1, $qfgetthumb_default_image);
        } else {
            $img = '';
        }
        $content .= $img;
...
/* thumbnail image */
function cf_qf_get_commodity_img($postid, $qfgetthumb_option_1, $qfgetthumb_default_image) {
    global $post;
    $post_pool = $post;
